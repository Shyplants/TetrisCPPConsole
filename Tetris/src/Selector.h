...
struct SelectorParams
{
	...
	SelectorParams(int x = 20, int y = 10, int columns = 1, int spacing_x = 20, int spacing_y = 5, std::wstring cursor = L"->")
		: x{ x }
		, y{ y }
		, columns{ columns }
		, currentX{ 0 }
	...
	, m_Rows{}
{
	...
	m_Rows = static_cast<int>(std::ceil(m_Data.size() / (params.columns == 0 ? 1 : params.columns)));
...
inline void Selector<T>::DrawItem(int x, int y, T item)
{
	...
	TETRIS_ERROR("Data type [" + type + "] - needs to be a wstring, Please create the DrawItem function to override!");
}
